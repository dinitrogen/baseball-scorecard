...
function calculateAVG(player) {
  let hits =
    player.numSingles + player.numDoubles + player.numTriples + player.numHRs;
  ...
  for (let player of myRoster) {
    ...
    moveDownButton.onclick = function () {
      ...
      if (myRoster.length === 1) {
        ...
        [myRoster[index], myRoster[index + 1]] = [
          myRoster[index + 1],
          myRoster[index],
        ];
      }
    ...
    moveUpButton.onclick = function () {
      ...
      if (myRoster.length === 1) {
        ...
        [myRoster[myRoster.length - 1], myRoster[index]] = [
          myRoster[index],
          myRoster[myRoster.length - 1],
        ];
      } else {
        [myRoster[index], myRoster[index - 1]] = [
          myRoster[index - 1],
...
function updatePlayer(player) {
  ...
  player.numStrikeouts = parseInt(
    document.getElementById("editStrikeouts").value
  );
